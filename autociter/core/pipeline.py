...
def get_text_from_url(url):
    ...
    try:
        ...
        allText = Extractor(
            extractor='KeepEverythingExtractor', url=url).getText()
        ...
        for i in range(1, 4):
            if extractedWords[-i] in totalWords:
                reverseLookupIndex = len(
                    totalWords) - 1 - totalWords[::-1].index(extractedWords[-i])
                totalWords = totalWords[:reverseLookupIndex]
                break
        print("Text scrape successfully finished in {0} seconds".format(
            time.time() - startTime))
...
def get_wikipedia_article_links_info(file, args):
    ...
    def get_attribute(r, arg):
        ...
        if arg in ['title', 'publisher', 'date', 'url', 'archive-url']:
            ...
            temp_list = [(r["first"], r["last"]), (r["first1"], r["last1"]),
                         (r["first2"], r["last2"])]
            validate_author = lambda x: x[0].strip() not in ["", "null"] and x[1].strip() not in ["", "null"]
            return list(
                set([' '.join(a) for a in temp_list if validate_author(a)]))
...
def save_data(fileName, data):
    ...
    for datapoint in data:
        ...
        saved_dict[
            datapoint['url']]['article_one_hot'] = datapoint['article_one_hot']
...
def vectorize_text(text, char_len=600):
    ...
    def pad_text(text, text_len=600):
        ...
        if len(text) < text_len:
            ...
            return text[:cur_len // 2] + (' ') * remainder + text_len[cur_len //
                                                                      2:]
        else:
            return text
